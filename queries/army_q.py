...
def get_one_garrison(cursor, garrison_id):
	query = "SELECT * FROM armies WHERE garrison = {0:d} LIMIT 1;".format(int(garrison_id))
...
def mass_get_army_monsters(cursor, army_dict):
	for k, the_army in army_dict.items():
		the_army.monsters = {}
	
	query = "SELECT * FROM army_monsters"
	try: cursor.execute(query)
	except Exception as e:
		raise Exception("Database error: %s\nQuery: %s" % (str(e.args[0]).replace("\n",""), query))
	for row in cursor:
		if row['army'] not in army_dict: continue
		army_dict[row['army']].monsters[row['monster']] = row['amount']
