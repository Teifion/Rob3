...
mission_order, mission_dict = mission_q.get_pending_missions()
...
for mission_id in mission_order:
	...
	if the_mission.state == 0:
		...
		handle = '<td style="padding: 0px;"><a class="block_link" href="web.py?mode=handle_mission&amp;mission=%d">Handle</a></td>' % mission_id
...
<tr class="row%(row_two)d">
	<td style="padding:0px;" colspan="7">
		<a href="list_missions" class="block_link">Show all missions</a>
	</td>
</tr>
""" % {	'row':			(count % 2),
		'row_two':		((count+1) % 2),
