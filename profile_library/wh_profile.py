...
def main(cursor, options):
	...
	# Setup the World
	the_wh = wh.Wh(the_world=the_world)
	the_wh.local_path = True
	the_wh.setup(true_team_list=team_list)
	...
	for t in team_list:
		...
		try:
			...
			output = the_wh.make_wh(t)
			
			md5_name = team_f.team_hash(the_team.name)
			f = open('%swh_%s.html' % (common.data['cache_path'], md5_name), 'w')
			f.write(output)
			f.close()
			...
			print("Error in making WH for team '%s'" % the_world._teams[t].name)
			raise
