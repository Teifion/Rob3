...
def sad_batch(w, verbose=True):
	...
	# print("Kruzen: %s" % str(", ".join([("%s: %s" % (r, round(a)) if a != 0 else "") for r, a in city_dict[451].goods.items()])))
...
def approve_buys(w, city_dict, verbose=True):
	...
	if verbose:	it = cli_f.progressbar(city_dict.items(), "Approving buys: ", 60, True)
	else:		it = city_dict.items()
...
def supply_and_demand(the_world):
	...
	batches = 0
	while not satisfied:
		sold = sad_batch(the_world)
		if sold <= 0:
			if config['hops'] < config['max hops']:
				config['hops'] += 3
				continue
			else:
				satisfied = True
...
def print_reports(the_world, report_dict, reports=['res_summary', 'production', 'demand', 'wealth']):
	...
	if 'res_summary' in reports:
		print(print_res_dict(report_dict['pre']['res_summary'],		the_world, "''Pre Summary''"), "\n")
		print(print_res_dict(report_dict['post']['res_summary'],		the_world, "''Post Summary''"), "\n")
	
	if 'res_surplus' in reports:
		print(print_res_dict(report_dict['pre']['res_surplus'],		the_world, "''Pre Surplus''"), "\n")
		print(print_res_dict(report_dict['post']['res_surplus'],		the_world, "''Post Surplus''"), "\n")
	
	if 'res_demand' in reports:
		print(print_res_dict(report_dict['pre']['res_demand'],		the_world, "''Pre Demand''"), "\n")
		print(print_res_dict(report_dict['post']['res_demand'],		the_world, "''Post Demand''"), "\n")
